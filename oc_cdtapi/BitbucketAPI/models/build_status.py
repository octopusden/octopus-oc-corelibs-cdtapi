...
class BuildStatus(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["state", "key", "name", "url", "description"]

    @field_validator('state')
    def state_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['SUCCESSFUL', 'FAILED', 'INPROGRESS']):
            raise ValueError("must be one of enum values ('SUCCESSFUL', 'FAILED', 'INPROGRESS')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of BuildStatus from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of BuildStatus from a dict"""
        if obj is None:
            return None
