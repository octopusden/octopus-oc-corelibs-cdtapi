...
class UserRole(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["user", "role", "approved", "status"]
    ...
    def role_validate_enum(cls, value):
        ...
        if value not in set(['PARTICIPANT', 'REVIEWER', 'AUTHOR']):
            raise ValueError("must be one of enum values ('PARTICIPANT', 'REVIEWER', 'AUTHOR')")
        return value
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(['UNAPPROVED', 'APPROVED']):
            raise ValueError("must be one of enum values ('UNAPPROVED', 'APPROVED')")
        return value
