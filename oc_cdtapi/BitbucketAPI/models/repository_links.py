...
from oc_cdtapi.BitbucketAPI.models.link import Link
...
class RepositoryLinks(BaseModel):
    ...
    RepositoryLinks
    """ # noqa: E501
    clone: Optional[List[Link]] = None
    var_self: Optional[List[Link]] = Field(default=None, alias="self")
    __properties: ClassVar[List[str]] = ["clone", "self"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RepositoryLinks from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in clone (list)
        _items = []
        if self.clone:
            for _item_clone in self.clone:
                if _item_clone:
                    _items.append(_item_clone.to_dict())
            _dict['clone'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in var_self (list)
        _items = []
        if self.var_self:
            for _item_var_self in self.var_self:
                if _item_var_self:
                    _items.append(_item_var_self.to_dict())
            _dict['self'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RepositoryLinks from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "clone": [Link.from_dict(_item) for _item in obj["clone"]] if obj.get("clone") is not None else None,
            "self": [Link.from_dict(_item) for _item in obj["self"]] if obj.get("self") is not None else None
        })
        return _obj
