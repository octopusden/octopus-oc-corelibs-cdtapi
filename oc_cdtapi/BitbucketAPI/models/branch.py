...
from oc_cdtapi.BitbucketAPI.models.branch_metadata import BranchMetadata
...
class Branch(BaseModel):
    ...
    type: StrictStr
    latest_commit: Annotated[str, Field(strict=True)] = Field(alias="latestCommit")
    latest_changeset: Annotated[str, Field(strict=True)] = Field(alias="latestChangeset")
    is_default: StrictBool = Field(alias="isDefault")
    metadata: Optional[BranchMetadata] = None
    __properties: ClassVar[List[str]] = ["id", "displayId", "type", "latestCommit", "latestChangeset", "isDefault", "metadata"]
    ...
    def latest_commit_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"[A-Fa-f0-9]+", value):
            raise ValueError(r"must validate the regular expression /[A-Fa-f0-9]+/")
        return value

    @field_validator('latest_changeset')
    def latest_changeset_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"[A-Fa-f0-9]+", value):
            raise ValueError(r"must validate the regular expression /[A-Fa-f0-9]+/")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "type": obj.get("type"),
            "latestCommit": obj.get("latestCommit"),
            "latestChangeset": obj.get("latestChangeset"),
            "isDefault": obj.get("isDefault"),
            "metadata": BranchMetadata.from_dict(obj["metadata"]) if obj.get("metadata") is not None else None
        })
        return _obj
