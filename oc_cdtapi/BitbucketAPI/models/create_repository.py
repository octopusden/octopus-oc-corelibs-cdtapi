...
class CreateRepository(BaseModel):
    ...
    """ # noqa: E501
    name: StrictStr
    scm_id: Optional[StrictStr] = Field(default='git', alias="scmId")
    forkable: Optional[StrictBool] = None
    default_branch: Optional[StrictStr] = Field(default=None, alias="defaultBranch")
    public: Optional[StrictBool] = None
    description: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["name", "scmId", "forkable", "defaultBranch", "public", "description"]

    @field_validator('scm_id')
    def scm_id_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['git', 'mercurial']):
            raise ValueError("must be one of enum values ('git', 'mercurial')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CreateRepository from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CreateRepository from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "name": obj.get("name"),
            "scmId": obj.get("scmId") if obj.get("scmId") is not None else 'git',
