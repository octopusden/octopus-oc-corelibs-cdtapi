...
class DmsAPI(API.HttpAPI):   # we use HttpAPI as a base class - the idea of HttpAPI is to use it as a skelet for new API clients
    ...
    _env_token = '_TOKEN'
    # now we have a separate Components Registry Service for components info obtaining
    _env_crs = '_CRS_URL'
    ...
    def __init__(self, *args, **argv):
        ...
        if not self.crs_root:
            raise DmsAPIError(
                "DMS API initialization failed. The components request url is not set")
        if token:
    ...
    def get_artifacts(self, component, version, ctype=None):
        ...
        for t in types:
            ...
            artifacts += self.get(req, headers=self.headers,
                                  verify=False).json()
            # also, requests has json parser, no need to re-invent it

        logging.debug('About to return an array of %d elements', len(
            artifacts))  # logging has its own format-string engine
        return artifacts
    ...
    def get_components(self):
        ...
        components = self.web.get(crs_request_url, verify=False).json()[
            'components']
        logging.debug('About to return an array of %d elements',
                      len(components))
        return components
    ...
    def get_gav(self, component, version, ctype, artifact, classifier=None):
        ...
        req = ['1', 'component', component, 'version',
               version, ctype, artifact, 'gav']
        if classifier:
        ...
        _gav = ':'.join([gav['groupId'], gav['artifactId'],
                        gav['version'], gav['packaging']])
    ...
    def get_types(self): return [
        'notes', 'distribution', 'report', 'static', 'documentation']
    ...
    def get_versions(self, component):
        ...
        versions = list(map(lambda x: x.get('version'), self.get(
            req, headers=self.headers).json().get('versions')))
