...
class DmsGetverAPI (API.HttpAPI):
    ...
    def __init__(self, *args, **argp):
        ...
        logging.debug(
            'Calling base class constructor for availability of HttpAPI methods')
    ...
    def get_distr(self, distr_id, distr_option):
        ...
        distr = None
        distr_state_info = self.get_distr_state_info_byid(
            distr_id, distr_option)
        if distr_state_info['state'] != 'READY':
            logging.debug('Distribution [%s] is in not ready state [%s]' % (
                distr_id, distr_state_info['state']))
        else:
            logging.debug(
                'Distribution [%s] is in ready state, downloading' % distr_id)
            url = posixpath.join('dms-getver', 'distribution',
                                 'id:%s' % distr_id, 'download')
            ...
            if distr_resp.status_code != 200:
                ...
                logging.debug(
                    'Fetched [%s] bytes from [%s]' % (len(distr), url))
    ...
    def get_distr_state_info_byid(self, distr_id, distr_option):
        ...
        if distr_option == 'full':
            ...
            url = posixpath.join('dms-getver', 'rest', 'api',
                                 '1', 'distribution', 'id:%s' % distr_id)
        else:
            logging.debug('Diff distr info requested')
            url = posixpath.join('dms-getver', 'rest', 'api',
                                 '1', 'distribution-difference', 'id:%s' % distr_id)
    ...
    def get_dms_gav(self, distr_id, distr_option):
        ...
        if distr_option == 'full':
            url = posixpath.join('dms-getver', 'rest', 'api',
                                 '1', 'distribution', 'id:%s' % distr_id, 'gav')
        else:
            url = posixpath.join('dms-getver', 'rest', 'api', '1',
                                 'distribution-difference', 'id:%s' % distr_id, 'gav')
        resp = self.get(url)
        if resp.status_code == 200:
            logging.debug('OK response from dms-getver')
            gav = resp.json()
            gav_text = '%s:%s:%s:%s' % (
                gav['groupId'], gav['artifactId'], gav['version'], gav['packaging'])
            logging.debug('Returning [%s]' % gav_text)
        else:
            logging.debug(
                'Error response [%s] from dms-getver' % resp.status_code)
            gav_text = None
    ...
    def get_dms_log_url(self, distr_id, distr_option):
        ...
        logging.debug('Request for url for [%s] distr [%s]' % (
            distr_option, distr_id))
        if distr_option == 'full':
            url = posixpath.join('dms-getver', 'rest', 'api',
                                 '1', 'distribution', 'id:%s' % distr_id, 'log')
        else:
            url = posixpath.join('dms-getver', 'rest', 'api',
                                 'distribution-difference', 'id:%s' % distr_id, 'log')
    ...
    def wait_for_state(self, distr_id, distr_option):
        ...
        while ela < int(self.wait_state_timeout):
            ...
            if not state in self.waiting_states:
                logging.debug(
                    'Distr [%s] is in exit state [%s]' % (distr_id, state))
                distr_state_info = dsi
                break
            logging.debug(
                'Distr [%s] is in waiting state [%s]' % (distr_id, state))
    ...
    def _create_distr_request_int(self, version=None, source_version=None, distr_type=None, client_filter=None):
        ...
        if source_version:
            ...
            url = posixpath.join('dms-getver', 'rest',
                                 'api', '1', 'distribution')
            req_parm['version'] = version
            req_parm['filter'] = client_filter

        logging.debug('request parameters: [%s]' %
                      json.dumps(req_parm, indent=4))
        resp = self.post(url, json=req_parm)

        # we shold have been expecting 202 here, but it's 200 somehow
        if resp.status_code != 200:
            logging.debug(
                'DMS responds with unexpected status code [%s]' % resp.status_code)
        ...
        if distr_state and distr_id:
            logging.debug('Created dist id [%s] in state [%s]' % (
                distr_id, distr_state))
    ...
    def _dumb_404(self, resp):
        ...
        try:
            ...
            logging.debug(
                'Failed to get json from dms response, returning False')
            return False
        if j['code'] == 'DMS-GETVER-40001':
            logging.debug(
                'Found a "dms not found" response code, returning True')
            return True
        else:
            logging.debug(
                'Json response processed, but no "dms not found" code detected, returning False')
            return False
    ...
    def _get_distr_state_info_int(self, url, parms=None):
        ...
        if resp.status_code != 200:
            logging.debug(
                'DMS reponds with status code [%s]' % resp.status_code)
        ...
        if distr_state and distr_id:
            distr_state_info = self._normalize_dsi(distr_state_info)
            logging.debug('Found dist id [%s] in state [%s]' % (
                distr_id, distr_state))
    ...
    def _get_distr_state_url(self, version, source_version=None, distr_type=None, client_filter=None):
        ...
        logging.debug('Request for url version [%s], type [%s], src_ver [%s]' % (
            version, distr_type, source_version))
        ...
        if source_version:
            ...
            url = posixpath.join('dms-getver', 'rest', 'api',
                                 '1', 'distribution', 'search')
            parms = {'filter': client_filter,
                     'product': distr_type, 'version': version}
        logging.debug('URL=[%s]' % url)
        return url, parms
    ...
    def _normalize_dsi(self, distr_state_info):
        ...
        if 'initialFilters' in distr_state_info.keys():
            logging.debug(
                'Diff distribution meta-info detected, add distOption=diff, filter, version')
            ...
            logging.debug(
                'Full distribution meta-info detected, only add distOption=full')
